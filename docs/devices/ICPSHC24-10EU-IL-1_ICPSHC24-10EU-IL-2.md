...
* `transition`: Controls the transition time (in seconds) of on/off, brightness changes. Defaults to `0` (no transition). The value must be a number with a minimum value of `0`
