...
#### On with timed off
Support depends on the light firmware. The firmware version is not displayed in Z2M 2.9.2-1.
When setting the state to ON, it might be possible to specify an automatic shutoff after a certain amount of time. To do this add an additional property `on_time` to the payload which is the time in seconds the state should remain on.
Additionally an `off_wait_time` property can be added to the payload to specify the cooldown time in seconds when the light will not answer to other on with timed off commands.
Some devices might require both `on_time` and `off_wait_time` to work
Examples : `{"state" : "ON", "on_time": 300}`, `{"state" : "ON", "on_time": 300, "off_wait_time": 120}`.
Some devices neither option is rejected, but neither the switch-on nor the timed switch-off is executed (tested with Z2M 2.9.2-1.).
